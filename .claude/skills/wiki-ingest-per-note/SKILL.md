...
- For relevant topics: create a new page or update an existing one.
  ...
  - **Conflict detection (QMD-driven).** Before creating or updating a page, run a QMD top-k similarity query (`mcp__plugin_qmd_qmd__query`) against existing wiki content for the same topic. If a high-similarity hit (k=3, threshold ~0.85) semantically *contradicts* the new content (different decision, different fact, different conclusion — not just overlapping wording), do NOT silently overwrite. Instead:
