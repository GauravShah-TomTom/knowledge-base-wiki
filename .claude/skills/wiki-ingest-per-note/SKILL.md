...
- For relevant topics: create a new page or update an existing one.
  ...
  - **[team-mode] Conflict detection.** Before creating or updating a page, run a top-k similarity query via `mcp__team-wiki__wiki_query` against existing wiki content for the same topic. If a high-similarity hit semantically *contradicts* the new content (different decision, different fact, different conclusion — not just overlapping wording), do NOT silently overwrite. Instead:
    1. Mark the affected page with a top-of-page `⚠️ CONFLICT` block that quotes both the existing passage and the new passage with their source links.
    2. Add frontmatter tag `contradiction: true`.
    3. Include the literal string `⚠️ CONFLICT` in the page-level batch log entry's `summary` field, so downstream consumers (CI auto-merge gate, PR reviewers) can detect it.
    Near-duplicate without contradiction → merge into the existing page (do not duplicate). Otherwise → write a new page as usual.
...
- **After finishing each note's Wiki pages** (immediately, before moving to the next note): append its log entry to the batch log file specified in your prompt. Write one JSON object per line — one for the original file, plus one for each converted Markdown file produced from it.
  - **The `file` field must always be the path that was in the batch** (i.e. the file you were asked to ingest). For plain `.md` notes this is the note itself. For converted files (`.eml`, `.vtt`, `.pdf`, etc.) this is the source file path — not the converted `.md`.
  - **Also write a second entry for the converted `.md`**, so that file is also marked as processed and won't be re-batched.
...
```
  Converted file (two entries — source file first, then the converted `.md`):
```json
...
```
  The same two-entry pattern applies to `.vtt`, `.pdf`, and any other non-Markdown source that is converted before ingestion.
